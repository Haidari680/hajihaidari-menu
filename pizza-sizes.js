/* 🍽️ ظرفیت و قیمت چندگانه غذاها — admin + customer */
(function(){
  const SIZE_LABELS={one:'👤 ظرفیت ۱ نفر',two:'👥 ظرفیت ۲ نفر',family:'👨‍👩‍👧‍👦 ظرفیت خانواده'};
  const SB_URL='https://bjpascssizuskiujnzvf.supabase.co',SB_KEY='sb_publishable_VMPe2QDMNfdwwAEAYQ2Y4A_3idOGTvrK';
  let foodSizes={},client=null;
  ...
  function sizesFor(f){
    ...
    return Object.entries({one:p.one_price,two:p.two_price,family:p.family_price})
      .filter(([,v])=>v!==null&&v!==undefined&&Number(v)>0)
      .map(([key,v])=>({key,label:SIZE_LABELS[key],price:Number(v)}));
  }
  ...
  function renderFoodSizes(){
    ...
    grid.innerHTML=d.length?d.map(f=>{
      ...
      const actions=sizes.length
        ?`<div class="food-size-title">📦 انتخاب ظرفیت و قیمت</div><div class="food-sizes">${sizes.map(s=>`<button class="food-size" type="button" onclick="addFoodSizeToCart(${f.id},'${s.key}')"><span>${esc(s.label)}</span><strong>${window.fa(s.price)} افغانی</strong></button>`).join('')}</div>`
        :`<div class="row"><span class="price">${window.fa(f.price)} افغانی</span><button class="add" ${f.stock_status==='soldout'?'disabled':''} onclick="add(${f.id})">+</button></div>`;
  ...
  async function adminPanel(){
    ...
    panel.innerHTML='<h3>📦 ظرفیت و قیمت چندگانه غذا</h3><p class="muted">برای هر غذا می‌توانی تا سه ظرفیت با قیمت جداگانه ثبت کنی. هر ظرفیتی که قیمت نداشته باشد برای مشتری نمایش داده نمی‌شود.</p><div class="field"><label>🍽️ انتخاب غذا</label><select id="pzFood"><option>در حال بارگذاری...</option></select></div><div class="pz-grid"><div class="field"><label>👤 ظرفیت ۱ نفر</label><input id="pzOne" type="number" min="0" placeholder="قیمت"></div><div class="field"><label>👥 ظرفیت ۲ نفر</label><input id="pzTwo" type="number" min="0" placeholder="قیمت"></div><div class="field"><label>👨‍👩‍👧‍👦 ظرفیت خانواده</label><input id="pzFamily" type="number" min="0" placeholder="قیمت"></div></div><button class="btn gold" id="pzSave" type="button">💾 ذخیره ظرفیت و قیمت</button><p id="pzMsg" class="statusmsg"></p>';
    app.appendChild(panel);
    const select=$('pzFood'),one=$('pzOne'),two=$('pzTwo'),family=$('pzFamily'),msg=$('pzMsg');
    async function load(){
      const foods=await db.from('foods').select('id,name,category_id,categories(name)').order('category_id').order('sort_order');
      if(foods.error){select.innerHTML='<option>خطا در بارگذاری غذاها</option>';return;}
      select.innerHTML=(foods.data||[]).map(f=>`<option value="${f.id}">${esc(f.categories?.name||'بدون دسته')} — ${esc(f.name)}</option>`).join('')||'<option value="">غذایی وجود ندارد</option>';
      await loadOne();
    }
    ...
    $('pzSave').onclick=async()=>{
      ...
      msg.textContent=r.error?'❌ '+r.error.message:'✅ ظرفیت‌ها و قیمت‌های غذا ذخیره شد.';
