(() => {
  ...
  async function load(db){const r=await db.from('site_settings').select('key,value').in('key',['hero_logo','hero_slides','hero_texts']);if(r.error)return;const o=Object.fromEntries((r.data||[]).map(x=>[x.key,x.value]));
    ...
    if(Array.isArray(slides)&&slides.length){document.querySelectorAll('.hero .slide').forEach((el,i)=>{const x=slides[i];if(!x)return;const url=x.url||x;if(!url)return;let im=el.querySelector('img');if(!im){im=document.createElement('img');el.appendChild(im)}im.src=url;im.width=1200;im.height=390;im.loading=i?'lazy':'eager';im.decoding='async';})}
