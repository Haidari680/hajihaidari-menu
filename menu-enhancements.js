(() => {
  ...
  const start=()=>{if(!window.supabase)return setTimeout(start,250);addCss();const db=window.sb||window.supabase.createClient(URL,KEY);load(db)};
  async function load(db){
    const r=await db.from('site_settings').select('key,value').in('key',['hero_logo','hero_slides','hero_texts']);
    if(r.error)return;
    const o=Object.fromEntries((r.data||[]).map(x=>[x.key,x.value]));
    if(o.hero_logo){
      const logo=document.querySelector('.logo');
      if(logo){const url=typeof o.hero_logo==='string'?o.hero_logo:o.hero_logo.url;if(url)logo.innerHTML='<img class="heroLogoImg" src="'+esc(url)+'" alt="لوگوی مجموعه حاجی حیدری"><small>طعم اصیل، تجربه‌ای ماندگار</small>'}
    }
    let slides=o.hero_slides;
    if(typeof slides==='string'){try{slides=JSON.parse(slides)}catch{}}
    if(Array.isArray(slides)&&slides.length){
      document.querySelectorAll('.hero .slide').forEach((el,i)=>{
        const x=slides[i];if(!x)return;const url=x.url||x;if(!url)return;
        let im=el.querySelector('img');
        if(!im){im=document.createElement('img');el.appendChild(im)}
        im.src=url;im.width=1200;im.height=390;im.loading=i===0?'eager':'lazy';im.fetchPriority=i===0?'high':'low';im.decoding='async';
      });
    }
    let texts=o.hero_texts;
    if(typeof texts==='string'){try{texts=JSON.parse(texts)}catch{}}
    if(texts&&typeof texts==='object'){
      const h=document.querySelector('.heroText h1'),p=document.querySelector('.heroText p');
      if(h&&texts.title)h.textContent=texts.title;
      if(p&&texts.subtitle)p.textContent=texts.subtitle;
    }
  }
  start();
})();
