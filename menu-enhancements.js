(() => {
  ...
  function optimizeImages(){const firstHeroSlide=document.querySelector('.hero .slide');document.querySelectorAll('img').forEach(im=>{if(im.dataset.optimized==='1')return;const raw=im.currentSrc||im.src;if(!raw||!raw.includes('/storage/v1/object/public/'))return;try{const u=new URL(raw);u.pathname=u.pathname.replace('/storage/v1/object/public/','/storage/v1/render/image/public/');u.searchParams.set('width',window.innerWidth<=650?'520':'900');u.searchParams.set('quality','70');u.searchParams.set('resize','contain');const hero=im.closest('.hero');const isFirstHero=hero&&im.closest('.slide')===firstHeroSlide;im.loading=isFirstHero?'eager':'lazy';im.decoding='async';im.fetchPriority=isFirstHero?'high':'low';im.src=u.toString();im.dataset.optimized='1'}catch{}})}
