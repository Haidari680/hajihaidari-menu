...
async function init(){
 if(!db||!$('foodList')) return;
 let n=0;
 const timer=setInterval(async()=>{
  n++;
  if(typeof window.openEdit==='function'&&typeof window.saveEdit==='function'){
...
function install(){
 ...
 window.openEdit=async function(id){
  originalOpen(id);
  const f=(window.foodsCache||[]).find(x=>Number(x.id)===Number(id));
  const old=$('pizzaAdminFix'); if(old) old.remove();
  if(!isPizza(f)) return;
  const box=document.createElement('div');
  box.id='pizzaAdminFix';
  box.style.cssText='margin:14px 0;padding:14px;border:2px solid #e4b84f;border-radius:14px;background:#fffaf0';
  ...
  if(r.error){$('editMsg').textContent='❌ خطا در خواندن قیمت پیتزا: '+r.error.message;return;}
  $('pizzaOnePrice').value=r.data?.one_price??'';
  $('pizzaTwoPrice').value=r.data?.two_price??'';
  $('pizzaFamilyPrice').value=r.data?.family_price??'';
 };
 window.openEdit.__pizzaFix=true;
 const originalSave=window.saveEdit;
 window.saveEdit=async function(){
  const id=Number($('editId').value);
  const f=(window.foodsCache||[]).find(x=>Number(x.id)===id);
  if(isPizza(f)&&$('pizzaAdminFix')){
   ...
   const r=await db.from('pizza_sizes').upsert(row,{onConflict:'food_id'});
   if(r.error){$('editMsg').textContent='❌ ذخیره قیمت پیتزا: '+r.error.message;$('editMsg').style.color='#b52b2b';return;}
  }
  await originalSave();
 };
 window.saveEdit.__pizzaFix=true;
